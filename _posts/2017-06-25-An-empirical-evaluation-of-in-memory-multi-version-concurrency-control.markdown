...
しかし，市場にある多くのMVCC DBMSはディスクベース・シングルコアを前提としたアーキテクチャで実装されており，
データベースの研究開発における現在のデファクトスタンダードである，インメモリ・メニーコア環境での性能はどうか？という点については議論が不足している．


この論文では，MVCCの実装における重要な要素を四分類し，それぞれについてインメモリ・メニーコア環境でのベンチマークを行っている．
...
このとき最大のボトルネックはディスクI/Oではなくキャッシュ競合や `malloc/free` といったメモリアクセスである．
このため，単一バージョンの[OCC](https://the-weekly-paper.github.io/jekyll/update/2017/06/11/on-optimistic-concurrency-control.html)が手法として選択されることが多い．
...
MVCCとは何か? という点について少し説明する．大枠は[Wikipedia](https://en.wikipedia.org/wiki/Multiversion_concurrency_control)の説明を読むと良い．

理論的な定式化が厳密になされている分野ではあるのだが，あえてざっくりした表現で説明すると，MVCCとは，その名の通り，「マルチバージョンのデータ構造を用いて同時実行制御を行う」ことである．

たとえば, 配列の各要素に直接構造体を詰め込んでデータを管理するのが単一バージョンの同時実行制御だとして, 配列の各要素がポインタになっていて，そこから線形リストで各更新ごとのバージョンを辿れるデータ構造がMVCCである．(もちろん，MVCCは理論的にはデータ構造の制約は存在しない．一例として線形リストを挙げた．）

この場合，更新がin-placeに行えない上，不要に線形リストを更新するため，MVCCのほうが空間的にも時間的にも計算量が多いことは明らかである．

しかし，一時間前から走りっぱなしの凄まじく長いバッチ処理やOLAPクエリといったものが直列化可能性に違反せずコミットでき，アドホックな分析クエリに強い，という様に，比較的柔軟にクエリを処理できる能力がMVCCにはある．勿論，トランザクションを直列化するためのロック等の使い方も変わってくる．


研究者にとっては無用なオーバヘッドが大きいとされるMVCCが，しかし依然として市場の大多数を支配していることは興味深い．
単にアカデミアとインダストリアルに乖離があるという見方もできるが，MVCCが持つ**広い実装上の選択肢とスケジュール空間**が実用上重要となってくるシーンは多い．

一方で，MVCCのDBMSを実装する際には，MVCCの理論からは直交した実装上の選択がいくつか存在する．
こうした選択は各DBMSの実装に留まる話であり，研究の世界で議論されてはこなかった．
アカデミアとインダストリアルの壁を埋める同時実行手法がやはりMVCCしかないなら，MVCCのオーバヘッドをどう削減していくか？という実装上の選択肢こそが
次の研究領域になるということも充分にあり得る．
...
もちろんこれはディスクベースでシングルコアの環境でのベンチマークで有り，
現在のトランザクション研究の前提となるインメモリ・メニーコア環境での結果とは大きく異なる可能性がある．
...
この図表は非常に面白い．今後ドラスティックな変更が無い限り数年間にわたって使い回せる表であると思われる．
...
大まかには，各タプルに `read timestamp` `write timestamp` を付与し，これを更新することで直列実行との等価性を検証する．
すなわち，`write ts < my_ts` のとき読めるし，`read_ts < my_ts` のとき書ける，といった様に(厳密には異なるが)規定される．読み書きはすべてタイムスタンプの更新を伴う．
...
This separation is unnecessary in an in-memory DBMS, thus with MV2PL the locks are embedded in the tuple headers.
```
